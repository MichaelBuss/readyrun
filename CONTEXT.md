...
- **Clack** — resolved: **Init** is the only prompt UI, interactive by default. `--answers` scripts **Init** without a TTY; it is not a command assembler in front of `run`. **Run** and **Doctor** liveness is stdout (stage heartbeat on a TTY, stage lines otherwise), not Clack.
