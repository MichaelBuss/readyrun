...
- **Init** is the only Clack prompt UI, and Clack is its default. `--answers <file>` is the scriptable path through the same writer; it is not a second prompt UI. `run` and `doctor` are flags plus stdout: on a TTY they show a stage-labelled heartbeat (Doctor, Frontier, Worktree, Worker) and an in-flight **Ticket** line (id, title, branch, started/cap); off a TTY those stages are newline-delimited lines with no spinner animation. There is no wizard that assembles a `run` command. Bare `readyrun` is usage, not a menu. An unattended **Run** must not prompt.
