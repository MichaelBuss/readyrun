...
14b. As a repo maintainer, I want **Doctor** to fail when the **Consumer**'s install output is neither tracked nor ignored, so that I find out before a **Run** hard-stops every **Ticket** for dirt ReadyRun created.
...
- Doctor also fails when the Consumer has a lockfile whose install output (`node_modules`) is neither tracked nor ignored, and names adding it to `.gitignore`, so a Run does not spend the first Ticket discovering that ReadyRun's own install looks like Worker dirt (ADR 0031).
